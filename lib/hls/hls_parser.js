...
/**
 * Updates a program date time.
 *
 * @param {!Array.<!shaka.hls.Tag>} tags
 * @private
 */
shaka.hls.HlsParser.prototype.updateProgramDateTime_ = function(tags) {
    const Utils = shaka.hls.Utils;
    let programDateTimeTag = Utils.getFirstTagWithName(tags,
        'EXT-X-PROGRAM-DATE-TIME');

    if (programDateTimeTag) {
        this.presentationTimeline_
            .setProgramDateTime(new Date(programDateTimeTag.value));
    }
};
...
shaka.hls.HlsParser.prototype.updateStream_ = function(streamInfo) {
  this.requestManifest_(streamInfo.absoluteMediaPlaylistUri).then(
    ...
    this.updateProgramDateTime_(playlist.tags);
...
shaka.hls.HlsParser.prototype.createSegmentReference_ =
  ...
  this.updateProgramDateTime_(tags);
